...
void button_init() {
	...
	GPIO_PORTE_DIR_R &= 0b000000; //Clear bits 5:0
	GPIO_PORTE_DEN_R |= 0b111111;
...
uint8_t button_getButton() {
    ...
    if(((GPIO_PORTE_DATA_R >> 0) & 0b1) == 0){
        return 1;
    }
    else if(((GPIO_PORTE_DATA_R >> 1) & 0b1) == 0){
        return 2;
    }
    else if(((GPIO_PORTE_DATA_R >> 2) & 0b1) == 0){
        return 3;
    }
    else if(((GPIO_PORTE_DATA_R >> 3) & 0b1) == 0){
        return 4;
    }
    else if(((GPIO_PORTE_DATA_R >> 4) & 0b1) == 0){
        _prevButton = 5;
    }
    else if(((GPIO_PORTE_DATA_R >> 5) & 0b1) == 0){
        return 6;
    }
	return 0; // REPLACE ME
}
