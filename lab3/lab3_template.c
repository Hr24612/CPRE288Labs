...
int8_t read_shaft_encoder(void) {
    ...
    // Compares old_value with new_value from Port B and sets rotation to 1 or -1, 1 for clockwise -1 for counter-clockwise.

    // If the knob was in a groove MIGHT BE REVERSED!!!!!!!!!
    if (old_value == 0b00000011) {          // Step 3: Enter here what you did for step 2.
        if (new_value == 0b00000010) {// What will new_value be if you're rotating CW out of a groove

            rotation = 1;
        }
          //rotation = rotation +1;
        if (new_value == 0b00000001) {      // What will new_value be if you're rotating CCW out of a groove
    ...
    old_value = new_value; // Save new_value into old value to pick up where it was at.
...
void  move_stepper_motor_by_step(int num_steps, int direction) {
    ...
    //Takes in port b data, stores first 4 bits in step, updates step based off of whether direction is 1 or -1, then or Port B with step again and wait 10 milliseconds.
    //Turns stepper motor based off of left 4 bits in Port B.

    uint8_t i;
    //taking in the portb data and keeping the first four bits.

    static char step = 0b00010000;
    GPIO_PORTB_DATA_R &= 0b00001111;
    for(i = 0; i < num_steps; i++){
        ...
        if(direction == 1 ){
            ...
            else if(step == 0b00100000){
                step = 0b01000000;
            }
            else if (step == 0b01000000){
               step = 0b10000000;
            }
            else if(step == 0b10000000){
               step = 0b00010000;
             }
        ...
        // Or with step to turn stepper motor without overwriting anything.
        GPIO_PORTB_DATA_R = ((GPIO_PORTB_DATA_R & 0x0F) | step);
...
int main(void) {
    ...
    while(1){
            ...
            temp = read_shaft_encoder();
            shaft += temp;
            uint8_t button = button_getButton();

            //Displays the button pressed and the running sum of encoder.
            lcd_printf("Button:%d, Shaft:%d", button, shaft);

            //used to check the direction.
            if(temp == -1 || temp == 1) {

                //int j;
                //j is used to set manipulate the movement of the motor with respect to the encoder.
                //for(j = 0; j < 1; j++) {

                    move_stepper_motor_by_step(100000, temp);
                //}
            }
